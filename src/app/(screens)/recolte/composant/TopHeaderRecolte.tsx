...
import { GREEN, GREEN_DARK, LIGHT_GRAY_COLOR, TEXT_COLOR } from '@/src/constants/Colors';
import Loading from '../../../../components/ui/login/Loading';


export type LoadingType =
    | 'affectationBacPaysant'
    | 'affectationBinParcelle'
    | 'PeseBac'
    | 'reception'
    | 'AffectationBacGrade'
    | 'affectationPaysanTable'
    | 'entreLavage'
    | 'sortieLavage'
    | 'entreEgrainage'
    | 'sortieEgrainage'
...
const TopHeaderRecolte: FC<Props> = ({
    ...
    const openScanner = () => {
        ...
        // ScanManager.beginContinuousScan()
    };

    return (
        <View>
            <View style={[!isKeyboardVisible && styles.borderBottomShadow, { minWidth: wp(100), maxWidth: wp(100) }]}>
                <View style={styles.borderBottomShadow}>
                    <View style={styles.header}>
                        ...
                        <View style={{ flexDirection: 'row' }}>
                            {isScanner && appContext.settingApp.current.scanMode === 'scanner' && (
                                <Pressable
                                    onPress={openScanner}
                                    style={({ pressed }) => ({
                                    ...
                                    <Image source={Icons.scan} style={{ height: hp(4), width: hp(4) }} />
                                </Pressable>
                            )}
                            {/* {data && data.length > 0 && (
                <Pressable
                  ...
                  <Image source={Icons.database} style={{height: hp(4), width: hp(4)}} />
                </Pressable>
              )} */}
                            ...
                            {process && (
                                <Pressable
                                    ...
                                    <Image source={Icons.icon_sync} style={{ height: hp(4), width: hp(4) }} />
                                </Pressable>
                            )}
                            ...
                            {search ? (
                                <Pressable
                                    ...
                                    <Image source={Icons.search_2x} style={{ height: hp(4), width: hp(4) }} />
                                </Pressable>
                            ) : (
                                <Pressable
                                    ...
                                    <Image source={Icons.home} style={{ height: hp(4), width: hp(4) }} />
                                </Pressable>
                            )}
                        </View>
                    </View>
                        ...
                        {subTitle && (
                            <View>
                                <Text style={styles.instruction}>
                                    {subTitle}{' '}
                                    {subTitleBold && (
                                        <Text style={[styles.bold, styles.instruction, { marginTop: -hp(2) }]}>
                                            {subTitleBold}
                                        </Text>
                                    )}
                                </Text>
            ...
            {isSearch && (
                <View style={{ marginVertical: hp(3), marginHorizontal: wp(7) }}>
                    <View
                        ...
                        <Image source={Icons.search_2x} style={{ height: hp(3), width: hp(3) }} />
                        <TextInput
                            ref={inputRef}
                            placeholder=""
                            value={searchText}
                    ...
                    {searchText && (
                        <View style={{ marginTop: 5 }}>
                            <Text style={[styles.subTytle, { fontWeight: 'bold', marginLeft: 3 }]}>
                                {t('AFFECTATION_BIN_PARCELLE:TEXT_RESULT')}: <Text style={styles.subTytle}>{countResult}</Text>
                            </Text>
                        </View>
                    )}
                </View>
